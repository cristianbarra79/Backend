...
log4js.configure({
    ...
    categories: {
      ...
      archivo: { appenders: ["miLoggerwarn","miLoggerConsole"], level: "warn" },
      archivo2: { appenders: ["miLoggererror", "miLoggerConsole"], level: "error" }      
    }
})
      
const consola = log4js.getLogger("consola");
const warns = log4js.getLogger("archivo");
const error = log4js.getLogger("archivo2");


warns.warn("prueba warn")
error.error("prueba error")
...
if(modo == "cluster" || modo == "CLUSTER"){
    ...
    app.get('/prueba', (req, res) => {
        consola.info("request /prueba")
        res.send(`Servidor express en ${PORT} - <b>PID ${process.pid}</b> - ${new Date().toLocaleString()}`)
    })


    app.get("*", (req, res) => {
        warns.warn("ruta inexistente")
        res.status(404).send('Not found');
        
    })

    httpServer.listen(PORT, () => consola.info("SERVER ON"));
    httpServer.on("error", err=>error.error(err));
